...
process{
    ...
    $Dependency = [Ordered]@{
        SigCheck = @{
            Command = @{
                'NoRecurse' = '& "<DEPENDENCYPATH>" /accepteula -nobanner -e -h -c "<LOCATION>"'
                'Recurse' = '& "<DEPENDENCYPATH>" /accepteula -nobanner -e -h -c -s "<LOCATION>"'
            }
            Path = @{
                '32-bit' = Join-Path -Path (Get-PRPath -Bin) -ChildPath 'sigcheck.exe'
                '64-bit' = Join-Path -Path (Get-PRPath -Bin) -ChildPath 'sigcheck64.exe'
            }
            TestPath = @((Join-Path -Path $RemoteStageDirectory -ChildPath 'sigcheck*.exe'))
        }
    }
    ...
    Invoke-Command -Session $Session -ScriptBlock {
        ...
        foreach ($Key in $Dependency.Keys) {
            ...
            if ($Using:Recurse){
                ...
                $Command = $Dependency.$Key.Command.Recurse -Replace '<DEPENDENCYPATH>',$DependencyPath -Replace '<LOCATION>',$Using:Location
            }

            if(!$Using:Recurse){
                # Create dependency command
                $Command = $Dependency.$Key.Command.NoRecurse -Replace '<DEPENDENCYPATH>',$DependencyPath -Replace '<LOCATION>',$Using:Location
            }
