...
.SYNOPSIS
    Plugin-Name: Analyze-MaliciousLocalUsers.ps1
    
.Description
	This plugin performs analysis on data retrieved from Hunt-MaliciousLocalUsers
...
process {
    ...
    $AnalysisDir = ("{0}\{1}\Configuration\LocalUsers\Analysis" -f (Get-PRPath -Output),$HuntName)
    $DataLocation = ("{0}\{1}\Configuration\LocalUsers\*Collect-LocalUsers*.csv" -f (Get-PRPath -Output),$HuntName)
    ...
    $Queries = @(
        ...
        @{
            Name = 'Stack_Group_User'
            Query = @"
            SELECT
                COUNT(Group, User) as ct,
                Group,
                User
            INTO
                <File>
