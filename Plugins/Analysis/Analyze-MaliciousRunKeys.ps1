...
.SYNOPSIS
    Plugin-Name: Analyze-MaliciousRunKeys.ps1
    
.Description
	This plugin performs analysis on data retrieved from Hunt-MaliciousRunKeys
...
process {
    ...
    $AnalysisDir = ("{0}\{1}\Persistence\RunKeys\Analysis" -f (Get-PRPath -Output),$HuntName)
    $DataLocation = ("{0}\{1}\Persistence\RunKeys\*Collect-RunKeys*.csv" -f (Get-PRPath -Output),$HuntName)
    ...
    $Queries = @(
        ...
        @{
            Name = 'Stack_Key_Property_Value'
            Query = @"
            SELECT
                COUNT(Key, Property, Value) as ct,
                Key,
                Property,
                Value
            INTO
...
"@
        ...
        @{
            Name = 'Stack_Property_Value'
            Query = @"
            SELECT
                COUNT(Property, Value) as ct,
                Property,
                Value
            INTO
                <File>
