...
process{
    ...
    $Dependency = [Ordered]@{
        ...
        AmCacheParser = @{
            ...
            Path = @{
                '32-bit' = Join-Path -Path (Get-PRPath -Bin) -ChildPath 'AmCacheParser.exe'
                '64-bit' = Join-Path -Path (Get-PRPath -Bin) -ChildPath 'AmCacheParser.exe'
            }
        }
    }
    ...
    foreach ($ArtifactDirectory in $ToProcess) {
        ...
        $Artifacts = Get-ChildItem -Force -Recurse -Path $ArtifactDirectory.FullName | Where-Object { $PSItem.Name -eq 'Amcache.hve' }
        ...
        foreach ($ArtifactPath in $Artifacts) {
            # Build the log file
            $LogFile = Join-Path -Path $AnalysisDirectory.FullName -ChildPath ('AmCacheParser_{0}_Log.txt' -f $ArtifactPath.Name)

            # Build the command
            $Command = $Dependency.AmCacheParser.Command -Replace '<DEPENDENCYPATH>',$Dependency.AmCacheParser.Path.$Architecture -Replace '<ARTIFACTPATH>',$ArtifactPath.FullName -Replace '<ANALYSISFOLDERPATH>',$AnalysisDirectory.FullName
