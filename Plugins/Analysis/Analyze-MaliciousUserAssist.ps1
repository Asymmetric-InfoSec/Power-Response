...
.SYNOPSIS
    Plugin-Name: Analyze-MaliciousUserAssist.ps1
    
.Description
	This plugin performs analysis on data retrieved from Hunt-MaliciousUserAssist
...
process {
    ...
    $AnalysisDir = ("{0}\{1}\Execution\UserAssist\Analysis" -f (Get-PRPath -Output),$HuntName)
    $DataLocation = ("{0}\{1}\Execution\UserAssist\*Collect-UserAssist*.csv" -f (Get-PRPath -Output),$HuntName)
    ...
    $Queries = @(
        ...
        @{
            Name = 'Stack_Type_UserAssist_GUID'
            Query = @"
            SELECT
                COUNT(Type, UserAssist, GUID) as ct,
                Type,
                UserAssist,
                GUID
            INTO
