...
.Description
    ...
    jumplist results from the current date. 
...
process{
    ...
    $Dependency = [Ordered]@{
        ...
        JLECmd = @{
            ...
            Path = @{
                '32-bit' = Join-Path -Path (Get-PRPath -Bin) -ChildPath 'JLECmd.exe'
                '64-bit' = Join-Path -Path (Get-PRPath -Bin) -ChildPath 'JLECmd.exe'
            }
        }
    }
    ...
    foreach ($ArtifactDirectory in $ToProcess) {
        ...
        foreach ($ArtifactPath in $Artifacts) {
            # Build the log file
            $LogFile = Join-Path -Path $AnalysisDirectory.FullName -ChildPath ('JLECmd_{0}_Log.txt' -f $ArtifactPath.Name)

            # Build the command
            $Command = $Dependency.JLECmd.Command -Replace '<DEPENDENCYPATH>',$Dependency.JLECmd.Path.$Architecture -Replace '<ARTIFACTPATH>',$ArtifactPath.FullName -Replace '<ANALYSISFOLDERPATH>',$AnalysisDirectory.FullName
