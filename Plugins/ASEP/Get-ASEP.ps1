...
process{
    ...
    if ($Architecture -eq "64-bit") {
            ...
            Invoke-Command -ComputerName $Computer -ScriptBlock { & C:\ProgramData\Autorunsc64.exe /accepteula -a * -h -nobanner -v -s -t -c}

        } elseif ($Architecture -eq "32-bit") {

            Invoke-Command -ComputerName $Computer -ScriptBlock { & C:\ProgramData\Autorunsc.exe /accepteula -a * -h -nobanner -v -s -t -c}

        } else {

            Write-Error ("There was a problem processing Autorunsc on {0}. No data was collected." -f $Computer)
