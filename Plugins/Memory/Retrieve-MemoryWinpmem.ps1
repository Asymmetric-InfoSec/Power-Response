...
process{
    ...
    $Dependency = [Ordered]@{
        Winpmem = @{
            Command = '& "<DEPENDENCYPATH> -o <OUTPUTPATH> --volume_format raw -dd -t'
            Path = @{
                '32-bit' = Join-Path -Path (Get-PRPath -Bin) -ChildPath 'winpmem.exe'
                '64-bit' = Join-Path -Path (Get-PRPath -Bin) -ChildPath 'winpmem.exe'
            }
            TestPath = @((Join-Path -Path $RemoteStageDirectory -ChildPath 'winpmem.exe'))
        }
        SevenZip = @{
    ...
    Invoke-Command -Session $Session -ScriptBlock {
        ...
        $Dependency = [Ordered]@{
            Winpmem = @{
                Command = '& "<DEPENDENCYPATH>" -o "<OUTPUTPATH>" --volume_format raw -dd -t'
                TestPath = @((Join-Path -Path $Using:RemoteStageDirectory -ChildPath 'winpmem.exe'))
            }
            SevenZip = @{
        ...
        foreach ($Key in $Dependency.Keys) {
            ...
            $Command = $Dependency.$Key.Command -Replace '<DEPENDENCYPATH>',$DependencyPath -Replace '<PATH>',($Path -Join ' ') -Replace '<OUTPUTPATH>',('{0}\memory.raw' -f $Using:RemoteStageDirectory)
